              ...
              if(debugLevel(4))  log.debug "resp = $resp"
			   ...
			   if (rdata) adata=(Map)new JsonSlurper().parseText(rdata)
				...
				if(resp && resp.status == 200 && adata){
                 ...
                 if(debugLevel(4))   log.debug "ndata = $ndata"
...
Boolean sendCmdToHusqvarna(String mowerId, Map data, Boolean isRetry=false, String uriend='actions'){
	...
	msg= sBLANK
	if(debugLevel(4)){
		msg+="in sendcmd http params -- ${deviceListParams} "
	}
	msg +="HTTPPOST "
	if(msg){
	...
	((List<String>)settings.mowers)?.each{String mower ->
		...
		if(srcMap){
            ...
            //lgk get next run time into attribute  
                ...
                def ed =0
            //srcMap.attributes.mower.errorCodeTimestamp
          ...
          //  log.warn "attrs are ${srcMap.attributes.mower}"
			...
			Boolean // = mst in [ 'ERROR', 'FATAL_ERROR', 'ERROR_AT_POWER_UP' ]
